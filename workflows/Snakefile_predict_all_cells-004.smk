configfile: "configs/config_predict_all_cells.yaml"

if config["method"] == "keras":
    out=expand("results/{tissue}/objects/full_annot_not_subset.h5ad", tissue=config["tissue"])
if config["method"] == "CellTypist":
    out=expand("results/{tissue}/objects/celltypist_prediction.h5ad", tissue=config["tissue"])
...
if config["method"] == "keras":
    ...
    rule aggregate_after_rule_2:
        ...
        conda:
            "scvi-env"
        output:
            "results/{tissue}/objects/full_annot_not_subset.h5ad"
        resources:
            mem=750000,
            queue='teramem',
            mem_mb=750000,
...
if config["method"] == "CellTypist":
    rule predict_celltypist:
        ...
        shell:
            r"""
            mkdir -p results/{wildcards.tissue}/objects

            python bin/994-assess_celltypist_autoannot.py \
                --h5_anndata {params.original_input_round1} \
