...
# Additionally, after performing this we also observe infiltrating B Cell plasma cells to have a disproportionately high number of genes detected
# Also, a population of Enterocytes with very low nGenes detected
# Apply a nMAD threshold as have done for MT%
...
# Select threshold and subset
nGene_mad_threshold = 2.5
subset_nGene_threshold = True
if subset_nGene_threshold == True:
    adata = adata[adata.obs.abs_n_genes_by_counts_nmads < nGene_mad_threshold]
    print("SUBSET BASED ON NMAD THRESHOLD OF 2.5")
...
# benchmark integration using various methods
bm = Benchmarker(
    adata,
...
# Optional: Benchmark the integration of samples against the confident ones only as well
# Load in the summary of annotation confidence (this is generated by the keras_confidence_vs_celtypist_rectum.py script)
common_conf = pd.read_csv(tabdir + "/keras_all_common_anno_conf.csv", index_col=0)
notproblem_annot = common_conf[common_conf.common_conf > 0.5].label
keep_problems = False
if keep_problems == False:
    adata = adata[adata.obs.label.isin(notproblem_annot)]
...
print(df)
# Save 
if keep_problems == True:
    df.to_csv(tabdir + "/integration_benchmarking.csv")
else:
    df.to_csv(tabdir + "/integration_benchmarking_confident_annotations_only.csv")
