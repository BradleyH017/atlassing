...
# Define some params
config_var=configs/config_post_workflow.yaml # Using the within lineage config (for round 2 analysis)
...
mkdir -p sm_logs_post 

# Copy config to results
cp $config_var results_round2/
...
snakemake -j 1000 \
    ...
    --cluster " mkdir -p 'sm_logs/cluster/${worfklow_prefix}_{rule}'; bsub -q {resources.queue} -R 'rusage[mem={resources.mem_mb}] select[mem>{resources.mem_mb}] span[hosts=1]' -M {resources.mem_mb} -n {resources.threads} -J '${worfklow_prefix}_{rule}.{wildcards}' -G ${group} -o 'sm_logs_post/cluster/${worfklow_prefix}_{rule}/{rule}.{wildcards}.%J-out' -e 'sm_logs_post/cluster/${worfklow_prefix}_{rule}/{rule}.{wildcards}.%J-err'" \
    -s workflows/Snakefile_post_workflow_analyses-999.smk \
    --until all \
    --rerun-triggers mtime \
    --dag | dot -Tpng > dags/dag_post_workflow.png
...
# bsub -M 2000 -a "memlimit=True" -R "select[mem>2000] rusage[mem=2000] span[hosts=1]" -o sm_logs_post/snakemake_master-%J-output.log -e sm_logs_post/snakemake_master-%J-error.log -q oversubscribed -J "snakemake_master" < submit_snakemake_post_workflow_analysis-999.sh 
