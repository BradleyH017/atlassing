...
def main():
    ...
    # If have the celltype__ prefix in this column, remove it
    adata.obs[col_to_sort] = adata.obs[col_to_sort].str.replace('^celltype__', '', regex=True)

    # Divide by the value of col_to_sort
    groups = np.unique(adata.obs[col_to_sort])
    # Manually override this
    groups = ["epithelial", "immune", "mesenchymal"]
    for g in groups:
        print(g)
        # Filter
        print("Filtering")
        ...
        # Save
        print("saving")
        temp.write_h5ad(f"{output_file}_{g}.h5ad")
