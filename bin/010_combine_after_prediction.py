...
def parse_options():    
    ...
    parser.add_argument(
        '-tsp', '--two_step_prediction',
        action='store',
        dest='two_step_prediction',
        help='Two step prediction. Determines how columns are recombined'
    )
...
def main():
    ...
    two_step_prediction = inherited_options.two_step_prediction
    ...
    if two_step_prediction:
        # Each cell will have only been through a single keras model at the second layer, so will only have a single predicted__celltype score
        ...
        # Each cell will have been through several keras models at the second later. So will have several predicted__celltype_score
        # Aggregation will therefore need to be done per sample
        samp_list # TO DO: Extract samples list from the fpaths
        for s in samp_list:
    ...
    # CHECK THIS - ALL CELLS SHOULD BE ANNOTATED
