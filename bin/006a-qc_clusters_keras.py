...
def simpsons_index(cluster):
    total_cells = cluster.sum()
    sum_ni_ni_minus_1 = sum(n_i * (n_i - 1) for n_i in cluster)
    if total_cells > 1:
        D = sum_ni_ni_minus_1 / (total_cells * (total_cells - 1))
    else:
        D = 0  # If there is only one cell, the diversity index is 0
    return D
...
def main():
    ...
    for f in file_paths:
        ...
        qc_cols = ['log_total_counts', 'log_n_genes_by_counts', 'pct_counts_gene_group__mito_transcript']
        for qc in qc_cols:
        ...
        #
        #
        # Calculate the proportion of samples per cluster
        proportions = adata.obs.groupby(res)['samp_tissue'].value_counts(normalize=True).reset_index(name="samp_tissue_contribution")
        count_non_zero = proportions[proportions['samp_tissue_contribution'] > 0].groupby(res).size().reset_index(name='non_zero_count')
        #
        # Also calculate the simpsons index per clusters
        counts = adata.obs.groupby(res)['samp_tissue'].value_counts().reset_index(name="samp_tissue_count")
        pivot_table = counts.pivot(index='samp_tissue', columns=res, values='samp_tissue_count')
        simpsons_indices = pivot_table.apply(simpsons_index, axis=0)
        simpsons_indices = pd.DataFrame(simpsons_indices).reset_index()
        simpsons_indices.columns = [res, "simpsonD"]
        #
        ...
        plt.clf()
        #
        # Number of contributing samples and simpsons index
        mcc[res] = mcc['cell_label'].astype(int)
        mcc = mcc.merge(simpsons_indices, on = res, how="left")
        mcc = mcc.merge(count_non_zero, on = res, how="left")
        mcc['prop_non_zero'] = mcc['non_zero_count']/len(np.unique(adata.obs['samp_tissue']))
        plt.figure(figsize=(8, 6))
        # Plot points that pass
        plt.scatter(mcc.loc[(mcc['MCC'].astype(float) > MCC_thresh),
                                'simpsonD'],
                    mcc.loc[(mcc['MCC'].astype(float) > MCC_thresh),
                                'MCC'].astype(float),
                    s=100, color='orange', label='Pass cluster QC')
        #
        # Scatter plot for other points (points will be blue)
        plt.scatter(mcc.loc[(mcc['MCC'].astype(float) <= MCC_thresh),
                    ...
                    s=100, color='lightblue', label='Fail cluster QC')
        #
        for i, txt in enumerate(mcc[res]):
            plt.text(mcc['simpsonD'][i], mcc['MCC'][i], str(txt), ha='center', va='center')
        #
        plt.title(f"{tissue} - {res} cluster QC - Simpsons index")
        plt.axhline(y = MCC_thresh, color = 'black', linestyle = '--', alpha = 0.5)
        plt.legend()
        plt.xlabel('Simpsons index')
        plt.ylabel('MCC')
        plt.savefig(f"{f}/MCC_vs_simpsons_index_{res}.png", bbox_inches='tight')
        plt.clf()
        #
        # Number of non-0 contributing samples
        plt.figure(figsize=(8, 6))
        # Plot points that pass
        plt.scatter(mcc.loc[(mcc['MCC'].astype(float) > MCC_thresh),
                                'prop_non_zero'],
        ...
        #
        for i, txt in enumerate(mcc[res]):
            plt.text(mcc['prop_non_zero'][i], mcc['MCC'][i], str(txt), ha='center', va='center')
        #
        plt.title(f"{tissue} - {res} cluster QC - nSamples")
        plt.axhline(y = MCC_thresh, color = 'black', linestyle = '--', alpha = 0.5)
        plt.legend()
        plt.xlabel('Proportion of samples that contribute')
        plt.ylabel('MCC')
        plt.savefig(f"{f}/MCC_vs_n_non0_{res}.png", bbox_inches='tight')
        plt.clf()
        #
        #
        # Min cells
    ...
    adata = adata[adata.obs['final_clusters'].isin(prop_sum_filtered_all['cell_label'].iloc[:,0].values)] # Filtering the clusters
    cols_to_keep = [col for col in adata.obs.columns if not col.startswith('leiden_')] # Removing unwanted columns
