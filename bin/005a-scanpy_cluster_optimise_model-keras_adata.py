...
# Modified from Bradley to use h5ad files as input and a more recent version of keras/tensorflow (no gpus)
...
import keras
#from keras.utils import np_utils - BH update for keras with tensorflow
...
# print(device_lib.list_local_devices())  # list of DeviceAttributes # I'm getting a CPU stuff here
# tf.config.list_physical_devices('GPU') # I'm getting an empty line here
# tf.test.is_built_with_cuda() - True 
# tf.test.is_gpu_available() False 
# With scarches:
# scarches says that tf.test.is_built_with_cuda() = False
...
def class_report(y_true, y_pred, classes, y_pred_proba=None):
    ...
    ## NEW LINE - ADDED THE PARAMETER NAMES
...
def fit_model_keras(
    ...
    X_std,
    y,
    keras_params, # Make all parameters modifiable by the input params
    n_epochs=100,
    batch_size=32,
    ...
    #
    # Training - BH update to include all keras_params
    ...
    y_test_proba = model.predict(X_test) # NEW LINE BECAUSE PREDICT PROBA DOESN'T EXIST ANYMORE 
...
def main():
    ...
    parser.add_argument(
        ...
        dest='h5',
        required=True,
        help='H5 AnnData file where clusters have been saved to cluster slot. It also assumes that neighbors have been pre-computed'
    )
    ...
    options.dict_add = 'method::leiden:::resolution::' + str(options.res)
    dict_add = {}
    if options.dict_add != '':
    ...
    # Set X and y
    num_dir = options.full_h5.count("/")
    print(f"num_dir: {num_dir}")
    if num_dir > 0:
        print(f"full_h5 file: {options.full_h5}")
        print("Getting expression from additional dataset")
        if 'UMAP_X_scVI' in adata.obsm.keys():
            has_umap = True
        ...
        if has_umap:
            umap = adata.obsm['UMAP_X_scVI']
    
        clusters = adata.obs[options.cluster_col]
        del adata
        # Load in the full adata and use that. Genes still subset for expression in > x cells
        adata = sc.read_h5ad(options.full_h5)
        # Subset for the cells with clusters
        adata = adata[adata.obs.index.isin(clusters.index)]
        if has_umap:
            adata.obsm['X_umap'] = umap
    
        adata.X = adata.layers['log1p_cp10k']
        # Merge with clusters (rows will match after subset)
        adata.obs["cluster"] = clusters
        adata.obs[options.cluster_col] = adata.obs["cluster"]
        # Now define X
        X = adata.X
    else:
        print("Getting expression from originally passed object")
    ...
    if sp.sparse.issparse(X):
        X = X.todense()
        X = np.asarray(X) # NEW LINE BECAUSE I NEED A NUMPY ARRAY INSTEAD OF MATRIX
    ...
    if options.grid_search:
        ...
        # Add in extra data
        # model_report['sparsity_l1'] = sparsity_l1 # BH edit
        if dict_add:
        ...
        # Add in extra data
        y_prob_df['sparsity_l1'] = keras_params['sparsity_l1__activity']
        if dict_add:
            for key, value in dict_add.items():
                y_prob_df[key] = value
        ...
        for i in ['AUC', 'f1-score', 'average_precision_score', 'MCC']:
            ...
            plt.xscale('log', base=10) # NEWLINE before it was basex
            fig.savefig(
