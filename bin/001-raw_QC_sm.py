...
def main():
    ...
    if "None" not in plot_within:
        ...
        for t in tissues:
            ...
            if nMad_directionality == "uni":
                cutoff = np.median(data) - (float(relative_nMAD_threshold) * mad)
                line_color = sns.color_palette("tab10")[int(np.where(tissues == t)[0])]
                sns.distplot(data, hist=False, rug=True, color=line_color, label=f'{t} (relative): {10**cutoff:.2f}')
                plt.axvline(x = cutoff, linestyle = '--', color = line_color, alpha = 0.5)
            else:
                cutoff_low = np.median(data) - (float(relative_nMAD_threshold) * mad)
                cutoff_high = np.median(data) + (float(relative_nMAD_threshold) * mad)
                line_color = sns.color_palette("tab10")[int(np.where(tissues == t)[0])]
                sns.distplot(data, hist=False, rug=True, color=line_color, label=f'{t} (relative): {10**cutoff_low:.2f}-{10**cutoff_high:.2f}')
                plt.axvline(x = cutoff_low, linestyle = '--', color = line_color, alpha = 0.5)
                plt.axvline(x = cutoff_high, linestyle = '--', color = line_color, alpha = 0.5)
        #
        plt.legend()
        ...
        plt.axvline(x = np.log10(min_nUMI), color = 'black', linestyle = '--', alpha = 0.5,  label=f"absolute: {min_nUMI}")
        plt.title(f"Absolute cut off (black): {min_nUMI}")
        plt.savefig(f"{qc_path}/raw_nUMI_per_tissue_thresholded.png", bbox_inches='tight')
        plt.clf()
    ...
    if use_relative_mad == "yes":
        ...
        plt.xlabel('log10(nUMI)')
        plt.axvline(x = np.log10(min_nUMI), color = 'black', linestyle = '--', alpha = 0.5,  label=f"absolute: {min_nUMI}")
        plt.title(f"Absolute cut off (black): {min_nUMI}. Window: {cutoff_low**10:.2f} - {cutoff_high**10:.2f}")
        plt.savefig(f"{qc_path}/raw_nUMI_nmad_thresholded_per_tissue.png", bbox_inches='tight')
        plt.clf()   
    ...
    if "None" not in plot_within:
        ...
        plt.savefig(f"{qc_path}/raw_nGene_per_tissue_thresholded.png", bbox_inches='tight')
        plt.clf()
    ...
    if use_relative_mad == "yes":
        ...
        plt.xlabel('log10(nUMI)')
        plt.axvline(x = np.log10(min_nGene), color = 'black', linestyle = '--', alpha = 0.5,  label=f"absolute: {min_nGene}")
        plt.title(f"Absolute cut off (black): {min_nGene}. Window: {10**cutoff_low:.2f} - {10**cutoff_high:.2f}")
        plt.savefig(f"{qc_path}/raw_nGenes_nmad_thresholded_per_tissue.png", bbox_inches='tight')
        plt.clf()  
    ...
    if "None" not in plot_within:
        ...
        for t in tissues:
            ...
            if nMad_directionality == "uni":
                cutoff = np.median(data) + (float(relative_nMAD_threshold) * mad)
                line_color = sns.color_palette("tab10")[int(np.where(tissues == t)[0])]
                sns.distplot(data, hist=False, rug=True, color=line_color, label=f'{t} (relative): {cutoff:.2f}')
                plt.axvline(x = cutoff, linestyle = '--', color = line_color, alpha = 0.5)
            else:
                cutoff_low = np.median(data) - (float(relative_nMAD_threshold) * mad)
                cutoff_high = np.median(data) + (float(relative_nMAD_threshold) * mad)
                line_color = sns.color_palette("tab10")[int(np.where(tissues == t)[0])]
                sns.distplot(data, hist=False, rug=True, color=line_color, label=f'{t} (relative): {cutoff_low:.2f}-{cutoff_high:.2f}')
                plt.axvline(x = cutoff_low, linestyle = '--', color = line_color, alpha = 0.5)
                plt.axvline(x = cutoff_high, linestyle = '--', color = line_color, alpha = 0.5)
            #
            plt.legend()
        ...
        plt.xlim(0,100)
        plt.savefig(f"{qc_path}/raw_MT_per_tissue_thresholded.png", bbox_inches='tight')
        plt.clf()
    ...
    if use_absolute_MT == "yes" and use_relative_mad == "no":
        ...
        plt.title(f"Absolute cut off (black): {absolute_max_MT}")
        ...
        for t in tissues:
            ...
            if nMad_directionality == "uni":
                cutoff = max(adata.obs['pct_counts_gene_group__mito_transcript'][adata.obs['MT_perc_keep'] == True])
                print(f"The minimum threshold for MT is: {cutoff}")
                sns.distplot(data, hist=False, rug=True, label=t)
                plt.axvline(x = cutoff, linestyle = '--', alpha = 0.5)
            else:
                cutoff_low = min(adata.obs['pct_counts_gene_group__mito_transcript'][adata.obs['MT_perc_keep'] == True])
                cutoff_high = max(adata.obs['pct_counts_gene_group__mito_transcript'][adata.obs['MT_perc_keep'] == True])
                print(f"The window for MT is: {cutoff_low} - {cutoff_high}")
                sns.distplot(data, hist=False, rug=True, label=t)
                plt.axvline(x = cutoff_low, linestyle = '--', alpha = 0.5)
                plt.axvline(x = cutoff_high, linestyle = '--', alpha = 0.5)
        ...
        plt.axvline(x = MTgut, color = 'green', linestyle = '--', alpha = 0.5,  label=f"absolute gut: {MTgut}")
        plt.axvline(x = MTgut, color = 'red', linestyle = '--', alpha = 0.5,  label=f"absolute blood: {MTblood}")
        plt.title(f"Absolute cut off (black): {absolute_max_MT}. Window: {cutoff_low:.2f} - {cutoff_high:.2f}")
        plt.savefig(f"{qc_path}/raw_pct_counts_gene_group__mito_transcript_nmad_thresholded_per_tissue.png", bbox_inches='tight')
        plt.clf()  
